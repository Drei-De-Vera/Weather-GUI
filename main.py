...
class WeatherApp(QWidget):
    ...
    def initUI(self):
        ...
        # Only set alignment directly for widgets that support it (e.g. QLabel, QLineEdit)
    ...
    def get_weather(self):
        ...
        api_key = "f987d6d6782fb430f0bf037e9f339c65"
        city = self.city_input.text() # to get the text
        url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={api_key}"

        try:
            response = requests.get(url)
            response.raise_for_status() #will raise exception if there is a HTTP errors
            data = response.json()
        ...
        except requests.exceptions.RequestException as req_error: #network problem, invalid URL
